# macOS Workload Baseline Toolkit

> Top-level OS picker + lifecycle overview: [../README.md](../README.md)

Capture a 1-2 week resource-usage baseline on any Mac (CPU + GPU + memory + Activity Monitor screenshots), then assemble it into a single PDF report.
...
That's it. The script downloads the toolkit to `~/.local/share/workload-baseline` and symlinks the `baseline` command into `/usr/local/bin` (will ask for your password to do so).
...
Or download the ZIP from [the GitHub page](https://github.com/Quattio/workload-baseline) (green "Code" button -> "Download ZIP"), unzip it, then `cd` into `macos/` and run `./install.sh`.
...
3. **Composition snapshot** -- live `ps` grouped into Browsers+automation / Communication+media / macOS+system.
...
`/usr/local/bin` may not be in your PATH. Either add it (`echo 'export PATH="/usr/local/bin:$PATH"' >> ~/.zshrc`), or run with the full path: `~/.local/share/workload-baseline/macos/bin/baseline help`.
...
```
macos/
├── bin/baseline                                  # CLI entry point
├── scripts/
│   ├── memory-snapshot.sh                        # one capture
│   └── build-baseline-pdf.sh                     # assemble PDF
├── launchd/
│   └── com.workload-baseline.snapshot.plist      # scheduled-capture template
